import {combatMethods} from './guard-combat.mjs';
import {clearPoint,clearSegment,findPath} from './guard-navigation.mjs';
...
export class Game {
 ...
 navigation(){this.nav=this.flow();this.navVersion=(this.navVersion||0)+1;this.paths=new WeakMap();}
 ...
 canBuild(kind,x,y){const s=this.state,b=BUILDINGS[kind];if(!b||!('hp'in b)||!['prep','battle'].includes(s.phase))return 'phase';if(!this.validTile(x,y)||key(x,y)===key(CORE.x,CORE.y)||ENTRANCES.some(p=>p.x===x&&p.y===y))return 'tile';if(s.buildings.some(b=>b.x===x&&b.y===y)||s.heroes.some(h=>Math.abs(h.x-x)<.68&&Math.abs(h.y-y)<.68||Math.abs(h.home.x-x)<.68&&Math.abs(h.home.y-y)<.68)||s.enemies.some(e=>e.hp>0&&Math.abs(e.x-x)<.68&&Math.abs(e.y-y)<.68))return 'occupied';if(s.gold<b.cost)return 'gold';if(b.solid){const nav=this.flow({x,y});if(!ENTRANCES.every(p=>nav.has(key(p.x,p.y)))||s.enemies.some(e=>e.hp>0&&!e.flying&&!nav.has(key(Math.round(e.x),Math.round(e.y))))||s.heroes.some(h=>!nav.has(key(Math.round(h.x),Math.round(h.y)))||!nav.has(key(Math.round(h.home.x),Math.round(h.home.y)))))return 'route';}return null;}
 ...
 moveHero(id,x,y){const s=this.state,h=s.heroes.find(h=>h.id===id),blocked=this.blocked();blocked.add(key(CORE.x,CORE.y));if(!h||h.hp<=0||!['prep','battle'].includes(s.phase)||!clearPoint({x,y},blocked))return false;const path=findPath(h,{x,y},blocked);if(!path.length)return false;h.action=null;h.home={x,y};this.paths.set(h,{target:{x,y},path,version:this.navVersion});return true;}
 effect(type,data){this.state.effects.push({type,ttl:.5,...data,duration:data.ttl??.5});}
 ...
 damage(e,amount,source,magic=false){if(e.hp<=0)return;let dmg=Math.max(1,amount-(magic?0:e.armor));if(source?.gear&&this.stats(source).effects.includes('freeze')&&e.slow>0)dmg*=1.25;e.hp-=dmg;e.hit=.13;this.effect('number',{x:e.x,y:e.y,value:Math.round(dmg),ttl:.65,color:magic?'#d0b4eb':'#fff0ca'});if(source?.gear&&source.hp>0&&this.stats(source).effects.includes('lifesteal'))source.hp=Math.min(this.stats(source).hp,source.hp+Math.min(dmg,Math.max(0,e.hp+dmg))*.18);if(e.hp<=0){const s=this.state;s.gold+=e.gold;s.kills++;if(source?.gear)source.kills++;this.effect('coin',{x:e.x,y:e.y,value:e.gold,ttl:.8});}}
 resolveAttack(h,target){const s=this.state,st=this.stats(h),magic=h.id==='mage'||h.id==='priest';const victims=h.id==='mage'?s.enemies.filter(e=>e.hp>0&&distance(e,target)<1.15):[target];for(const e of victims){this.damage(e,st.atk,h,magic);if(st.effects.includes('freeze'))e.slow=2.3;if(st.effects.includes('burn'))e.burn=3;}
 ...
 if(st.effects.includes('chain')){const others=s.enemies.filter(e=>e.hp>0&&e!==target&&distance(e,target)<2.6).sort((a,b)=>distance(a,target)-distance(b,target)).slice(0,2);for(const e of others){this.launch(target,e,{kind:'chain',source:h.id,amount:st.atk*.65,magic:true,color:'#abdcfa'});}}
 }
 resolveSkill(h,x,y){const s=this.state,id=h.id,st=this.stats(h);if(id==='knight'){x=h.x;y=h.y;for(const e of s.enemies)if(e.hp>0&&!e.flying&&distance(e,h)<2.4){this.damage(e,st.atk*3.3,h);e.slow=2;}this.effect('burst',{x,y,ttl:.7,radius:2.4,color:'#e9bc74'});}else if(id==='ranger'){for(const e of s.enemies)if(e.hp>0&&distance(e,{x,y})<2.4)this.damage(e,st.atk*4.2,h);this.effect('burst',{x,y,ttl:.35,radius:2.4,color:'#d5e8a0'});}else if(id==='mage'){s.fields.push({x,y,ttl:5,tick:0,atk:st.atk*.45,source:id});this.effect('burst',{x,y,ttl:.8,radius:2.5,color:'#a9e0eb'});}else{for(const hero of s.heroes)if(hero.hp>0)hero.hp=Math.min(this.stats(hero).hp,hero.hp+100*(1+st.heal));for(const b of s.buildings)if(distance(b,{x,y})<3)b.hp=Math.min(b.maxHp,b.hp+65);this.effect('heal',{x,y,ttl:1.3,radius:3});}return true;}
 walk(h,dest,dt){const blocked=this.blocked();if(h.gear)blocked.add(key(CORE.x,CORE.y));let plan=this.paths.get(h);if(!plan||plan.version!==this.navVersion||distance(plan.target,dest)>.15){plan={target:{x:dest.x,y:dest.y},path:findPath(h,dest,blocked),version:this.navVersion};this.paths.set(h,plan);}while(plan.path.length&&distance(h,plan.path[0])<.035)plan.path.shift();if(plan.path.length)this.stepToward(h,plan.path[0],h.speed*dt);}
 tickPreparation(dt){this.state.time+=dt;for(const h of this.state.heroes){h.moving=false;if(h.hp>0&&distance(h,h.home)>.015)this.walk(h,h.home,dt);}for(const ef of this.state.effects)ef.ttl-=dt;this.state.effects=this.state.effects.filter(e=>e.ttl>0);}
 stepToward(e,p,step){const d=distance(e,p);if(d<.001)return;const f=Math.min(1,step/d);const dx=p.x-e.x;e.x+=dx*f;e.y+=(p.y-e.y)*f;e.moving=true;if(Math.abs(dx)>.01)e.face=dx>=0?1:-1;}
 tick(dt){const s=this.state;if(!Number.isFinite(dt)||dt<=0)return;dt=clamp(dt,0,.05);if(!['prep','battle'].includes(s.phase))return;for(const a of [...s.heroes,...s.enemies]){a.prevX=a.x;a.prevY=a.y;}if(s.phase==='prep'){this.tickPreparation(dt);return;}if(s.phase!=='battle')return;s.time+=dt;s.waveTime+=dt;this.advanceCombat(dt);for(const h of s.heroes){h.deathAge=h.hp<=0?(h.deathAge||0)+dt:0;h.cool=Math.max(0,h.cool-dt);h.attack=Math.max(0,h.attack-dt);h.anim=Math.max(0,h.anim-dt);h.moving=false;}for(const ef of s.effects)ef.ttl-=dt;s.effects=s.effects.filter(e=>e.ttl>0);while(s.queue.length&&s.queue[0].at<=s.waveTime){const e=s.queue.shift();this.spawn(e.kind,e.lane);}
 for(const b of s.buildings){b.ready=Math.max(0,b.ready-dt);b.attack=Math.max(0,b.attack-dt);if(b.ready>0||b.hp<=0)continue;const base=BUILDINGS[b.kind];if(b.kind==='frost'){for(const e of s.enemies)if(e.hp>0&&!e.flying&&distance(e,b)<1.2+b.level*.28)e.slow=Math.max(e.slow,.3);}if(b.kind==='tower'&&b.attack<=0){const targets=s.enemies.filter(e=>e.hp>0&&distance(e,b)<base.range+.25*(b.level-1));const target=targets.find(e=>e.id===s.focus)||targets.sort((a,b)=>distance(a,CORE)-distance(b,CORE))[0];if(target){this.launch(b,target,{kind:'tower',amount:base.atk*(1+.6*(b.level-1)),color:'#e9c578',tower:true});b.attack=base.rate;}}}
 for(const h of s.heroes){if(h.hp<=0||h.action)continue;const st=this.stats(h);let available=s.enemies.filter(e=>e.hp>0&&(h.id!=='knight'||!e.flying));if(h.id==='knight'){const near=available.filter(e=>distance(e,h.home)<3.4).sort((a,b)=>distance(a,h)-distance(b,h));const target=near.find(e=>e.id===s.focus)||near[0];if(target&&distance(h,target)>st.range*.85)this.walk(h,target,dt);else if(!target&&distance(h,h.home)>.025)this.walk(h,h.home,dt);}else if(distance(h,h.home)>.025)this.walk(h,h.home,dt);
 if(h.id==='priest'&&h.attack<=0){const hurt=s.heroes.filter(a=>a.hp>0&&a.hp<this.stats(a).hp-8&&distance(a,h)<st.range).sort((a,b)=>a.hp/this.stats(a).hp-b.hp/this.stats(b).hp)[0];if(hurt){this.beginAction(h,'heal',hurt);h.attack=h.rate/(1+st.haste);continue;}}
 ...
 if(e.action)continue;let target=null;if(e.siege){target=s.buildings.filter(b=>b.hp>0&&distance(b,e)<3.5).sort((a,b)=>distance(a,e)-distance(b,e))[0];}
 if(!target)target=s.heroes.filter(h=>h.hp>0&&distance(h,e)<(e.flying?.65:1.12)).sort((a,b)=>distance(a,e)-distance(b,e))[0];
 if(target){if(distance(e,target)>1.05){this.stepToward(e,target,e.speed*dt*(e.slow>0?.52:1));}else if(e.attack<=0){this.beginAction(e,'enemy',target);e.attack=1.15;}continue;}
 if(distance(e,CORE)<.7){if(e.attack<=0){this.beginAction(e,'core',s.core);e.attack=1.2;}continue;}
 if(e.flying)this.stepToward(e,CORE,e.speed*dt);else{if(!e.step||e.navVersion!==this.navVersion||distance(e,e.step)<.06){const route=this.route(e.x,e.y);const blocked=this.blocked();e.step=route[1]||route[0];for(let i=Math.min(route.length-1,8);i>=1;i--)if(clearSegment(e,route[i],blocked)){e.step=route[i];break;}e.navVersion=this.navVersion;}if(e.step)this.stepToward(e,e.step,e.speed*dt*(e.slow>0?.52:1));}
 }
 const dead=s.buildings.some(b=>b.hp<=0);s.buildings=s.buildings.filter(b=>b.hp>0);if(dead)this.navigation();s.enemies=s.enemies.filter(e=>e.hp>0||(e.deathAge=(e.deathAge||0)+dt)<.36);if(s.focus&&!s.enemies.some(e=>e.hp>0&&e.id===s.focus))s.focus=null;if(s.core.hp<=0){this.clearCombat();s.phase='defeat';s.result={wave:s.wave,kills:s.kills};return;}if(!s.queue.length&&!s.enemies.length&&!s.projectiles.length&&!s.heroes.some(h=>h.action))this.finishWave();}
 ...
 checkpoint(){if(!['prep','reward'].includes(this.state.phase))return null;const s=this.snapshot();s.enemies=[];s.effects=[];s.fields=[];s.queue=[];s.projectiles=[];s.events=[];for(const h of s.heroes){h.action=null;delete h.prevX;delete h.prevY;}return s;}
 static restore(data){
  ...
  try {
   ...
   const position=p=>p&&finite(p.x,0,W-1)&&finite(p.y,0,H-1);
   ...
   for(const hero of s.heroes){
    ...
    if(!h||!position(h.home)||stone.has(key(Math.round(h.home.x),Math.round(h.home.y)))||key(Math.round(h.home.x),Math.round(h.home.y))===key(CORE.x,CORE.y)||!h.gear||!finite(h.cool,0,60)||!integer(h.kills,0,100000))return null;
   ...
   for(const b of data.buildings){
    ...
    const k=key(b.x,b.y);
    if(tiles.has(k)||stone.has(k)||k===key(CORE.x,CORE.y)||ENTRANCES.some(e=>e.x===b.x&&e.y===b.y)||base.solid&&s.heroes.some(h=>Math.abs(h.home.x-b.x)<.66&&Math.abs(h.home.y-b.y)<.66))return null;
   ...
   g.navigation();
   if(!ENTRANCES.every(p=>g.nav.has(key(p.x,p.y)))||s.heroes.some(h=>!g.nav.has(key(Math.round(h.x),Math.round(h.y)))||!clearPoint(h,g.blocked())))return null;
   return g;
  }catch{return null;}
 }
}
